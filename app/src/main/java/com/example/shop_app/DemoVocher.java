...
public class DemoVocher extends AppCompatActivity {
    Chip confirm;
    ImageButton back;
    TextView name, des;
    ImageView img;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.demo_vocher);
        setControl();
        setData();
        setEvent();
    }
    ...
    private void setEvent() {
        ...
        confirm.setOnClickListener(view -> {
            if(check())  {
                ...
                database.child("demo").child(pr.getId()).setValue(size_stock);
                Toast.makeText(this,"Thêm sản phẩm: "+ pr.getName()+ " vào đơn hàng thành công", Toast.LENGTH_SHORT).show();
                back.requestFocus();
            }
        });
    }
    ...
    private void setData() {
        ...
        Glide.with(DemoVocher.this).load(pr.getImage()).into(img);
        ...
        database.child("productSize").child(pr.getId()).addValueEventListener(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(DemoVocher.this,"không thể kết nối tới dữ liệu",Toast.LENGTH_SHORT).show();

            }
        });
        database.child("demo").child(pr.getId()).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists()){
                    for(DataSnapshot dataSnapshot : snapshot.getChildren()){
    ...
    private void setControl() {
        back = findViewById(R.id.demoPr_back);
        img  = findViewById(R.id.demoPr_picture);
        confirm = findViewById(R.id.demoPr_confirm);
        name = findViewById(R.id.demoPr_name);
        des = findViewById(R.id.demoPr_des);
        txtM = findViewById(R.id.demoPr_M);
        txtL = findViewById(R.id.demoPr_L);
        txtXL = findViewById(R.id.demoPr_XL);
        txtXXL = findViewById(R.id.demoPr_XXL);
    }
}